...
export default function videoDataPlugin() {
  return {
    ...
    async load(id) {
      ...
      const firstAdaIndex = entries.findIndex(
        (entry) => entry[1].id === firstAdaVid,
      );
      const firstSurmaIndex = entries.findIndex(
        (entry) => entry[1].id === firstSurmaVid,
      );
      const firstPaulIndex = entries.findIndex(
        (entry) => entry[1].id === firstPaulVid,
      );
